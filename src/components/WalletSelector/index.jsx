...
export default function WalletSelector({ onClose }) {
	...
	return (
		<Container>
				...
				{walletList.map(({
					...
					return (
						<Card key={'wallet-connector-id' + id} onClick={handleClick}>
							<Icon src={require('../../assets/images/wallets/' + icon).default} />
							<Name>{withRedirect ? 'Install ' : ''}{name}</Name>
						</Card>
					)
				})}
			</Grid>
			...
			<RedirectButton onClick={redirectToEthereum}>Learn more about wallets</RedirectButton>
		</Container>
	)
}
